...
public class LocalCompanyBannerStorage(IWebHostEnvironment environment) : ICompanyBannerStorage
{
    ...
    {
        var extension = Path.GetExtension(file.FileName).ToLowerInvariant();
        var fileName = $"company_{Guid.NewGuid():N}{extension}";

        var relativePath = Path.Combine("uploads", "company-banners", fileName).Replace('\\', '/');
